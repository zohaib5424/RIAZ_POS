    ...
    public partial class StaffDocuments : Form
    {
        ...
        public void filldata()
        {
            try
            {
                ...
                string query = @"select * from Staff_Documents where status !='-1'";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (textBox1.Text.Trim() != "" && textBox1.Text.Trim().Length > 0)
                {
                    ...
                    string query = @"select max(cast(id as int)) from Staff_Documents";
                    SqlCommand cmd = new SqlCommand(query, con);
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    ...
                    query = @"select * from Staff_Documents where Document_Name='" + data2 + "' and status !='-1'";
                    ...
                    if (dt.Rows.Count <= 0)
                    {
                        query = @"insert into Staff_Documents values('" + id + "','" + data2 + "','1')";
                        cmd = new SqlCommand(query, con);
                        cmd.ExecuteNonQuery();
                        textBox1.Text = "";
                    ...
                    {
                        MessageBox.Show("Document Name Already Exist.");
                    }
                }
                else
                {
            ...
            try
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    if (textBox1.Text.Trim().Length > 0)
                    {
                        ...
                        string query = @"select * from Staff_Documents where Document_Name='" + data2 + "' and status !='-1'";
                        SqlCommand cmd = new SqlCommand(query, con);
                        SqlDataAdapter sda = new SqlDataAdapter(cmd);
                        DataTable dt = new DataTable();
                        sda.Fill(dt);
                        ...
                        if (dt.Rows.Count <= 0)
                        {
                            query = @"update Staff_Documents set Document_Name='" + data2 + "' where id = '" + dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells[0].Value.ToString().Trim() + "'";
                            cmd = new SqlCommand(query, con);
                            cmd.ExecuteNonQuery();
                    ...
                    {
                        MessageBox.Show("Enter Document Name For Update");
                    }
                }
                else
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    string query = @"update Staff_Documents set status='-1' where id = '" + dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells[0].Value.ToString().Trim() + "'";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.ExecuteNonQuery();
