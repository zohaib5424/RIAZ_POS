    ...
    public partial class ClassType : Form
    {
        ...
        public void filldata()
        {
            try
            {
                ...
                string query = @"select * from class_type where status!='-1'";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (textBox1.Text.Trim() != "" && textBox1.Text.Trim().Length > 0)
                {
                    ...
                    string query = @"select max(cast(id as int)) from class_type";
                    SqlCommand cmd = new SqlCommand(query, con);
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    ...
                    query = @"select * from Class_Type where class_type='" + data2 + "' and status !='-1'";
                    ...
                    if (dt.Rows.Count <= 0)
                    {
                        query = @"insert into Class_Type values('" + id + "','" + data2 + "','1')";
                        cmd = new SqlCommand(query, con);
                        cmd.ExecuteNonQuery();
                        textBox1.Text = "";
            ...
            try
            {
                ...
                if (e.ColumnIndex == 2)
                {
                    if (dataGridView1.Rows[e.RowIndex].Cells[2].Value.ToString() == "Enable")
                    {
                        ...
                        string query = @"update Class_Type set status='1' where id = '" + dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString().Trim() + "'";
                        SqlCommand cmd = new SqlCommand(query, con);
                        cmd.ExecuteNonQuery();
                    ...
                    else
                    {
                        ...
                        string query = @"update Class_Type set status='0' where id = '" + dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString().Trim() + "'";
                        SqlCommand cmd = new SqlCommand(query, con);
                        cmd.ExecuteNonQuery();
            ...
            try
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    if (textBox1.Text.Trim().Length > 0)
                    {
                        ...
                        string query = @"select * from Class_Type where class_type='" + data2 + "' and status !='-1'";
                        SqlCommand cmd = new SqlCommand(query, con);
                        SqlDataAdapter sda = new SqlDataAdapter(cmd);
                        DataTable dt = new DataTable();
                        sda.Fill(dt);
                        ...
                        if (dt.Rows.Count <= 0)
                        {
                            query = @"update class_type set class_type='" + data2 + "' where id = '" + dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells[0].Value.ToString().Trim() + "'";
                            cmd = new SqlCommand(query, con);
                            cmd.ExecuteNonQuery();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    string query = @"update class_type set status='-1' where id = '" + dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells[0].Value.ToString().Trim() + "'";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.ExecuteNonQuery();
